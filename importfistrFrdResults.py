...
def read_frd_result(
    ...
    Console.PrintMessage(
        "Read fistr results from frd file: {}\n"
        .format(frd_input)
    )
    inout_nodes = []
    ...
    for line in frd_file:
        ...
        if elements_found and (line[1:3] == "-2"):
            ...
            if elemType == 1:
                ...
                # first line
                # C3D20 FrontISTR --> hexa20 FreeCAD
                ...
                input_continues = False
                """
                FrontISTR uses a different node order in
                input file *.inp and result file *.frd for hexa20 (C3D20)
                according to Guido (the developer of fistr):
                see note in in first line of cgx manuel part element types
                fistr (and thus the *.inp) follows the ABAQUS convention
                documented in the fistr-documentation
                ...
                # first line
                # C3D15 FrontISTR --> penta15 FreeCAD
                ...
                # C3D10 FrontISTR --> tetra10 FreeCAD
                ...
                # B31 FrontISTR --> seg2 FreeCAD
                # N1, N2
                nd1 = int(line[3:13])
                nd2 = int(line[13:23])
                elements_seg2[elem] = (nd1, nd2)
            elif elemType == 12:
                # B32 FrontISTR --> seg3 FreeCAD
        ...
        if mode_stress_found and (line[1:3] == "-1"):
            ...
            # FrontISTR frd files: (Sxx, Syy, Szz, Sxy, Syz, Szx)
        ...
        if mode_strain_found and (line[1:3] == "-1"):
            ...
            # FrontISTR frd files: (Exx, Eyy, Ezz, Exy, Eyz, Ezx)
